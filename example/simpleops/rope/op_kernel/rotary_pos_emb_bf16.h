...
public:
    __aicore__ inline RopeBf16(SimpleOps::RopeTilingData *tilingData)
        :RopeBase<QK_DTYPE, COS_DTYPE, IF_COS_BROADCAST>(tilingData)
    {
        ...
        this->pipe_.InitBuffer(qkfp32QueueCO2_, 1, (this->tilingData_->maxUbSize -
            (this->batchSize_  + this->maxProcessNum_) * sizeof(half))); // 留給fp32的
    ...
    {
        vconv_bf162f32(qkfp32Ubuf_ + this->cosPad_, commonUbuf_ + this->cosPad_,  repeatTimes, 1,
            1, DEFAULT_REPEAT_STRIDE, DEFAULT_REPEAT_STRIDE / NUM_TWO);
        vconv_bf162f32(qkfp32Ubuf_ + this->sinPad_, commonUbuf_ + this->sinPad_,  repeatTimes, 1,
            1, DEFAULT_REPEAT_STRIDE, DEFAULT_REPEAT_STRIDE / NUM_TWO);
    }

    __aicore__ inline void CastB162F32(uint32_t repeatTimes1)
    {
        vconv_bf162f32(qkfp32Ubuf_ + oriPosF32_, commonUbuf_ + this->oriPos_, repeatTimes1, 1,
            1, DEFAULT_REPEAT_STRIDE, DEFAULT_REPEAT_STRIDE / NUM_TWO);
        vconv_bf162f32(qkfp32Ubuf_ + removeBeforeF32_, commonUbuf_ + this->removeBefore_,  repeatTimes1, 1,
            1, DEFAULT_REPEAT_STRIDE, DEFAULT_REPEAT_STRIDE / NUM_TWO);
        vconv_bf162f32(qkfp32Ubuf_ + PadBeforeF32_, commonUbuf_ + this->padBefore_,  repeatTimes1, 1,
            1, DEFAULT_REPEAT_STRIDE, DEFAULT_REPEAT_STRIDE / NUM_TWO);
    }

    __aicore__ inline void CastF322B16(__gm__ QK_DTYPE *dst,
        __ubuf__ QK_DTYPE *src1,
        __ubuf__ float *src,
        uint32_t repeatTimes1,
        uint32_t hiddenSize1)
    {
        vconv_f322bf16r(src1, src, repeatTimes1, 1,
