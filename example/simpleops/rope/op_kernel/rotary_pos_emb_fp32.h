...
public:
    __aicore__ inline RopeFp32(SimpleOps::RopeTilingData *tilingData, AscendC::TPipe *pipe)
    ...
    __aicore__ inline void Process(__gm__ uint8_t *extra)
    {
        ...
        for (uint32_t zz = 0; zz < this->dynamicRound_; ++zz) { // 每个核 核内只需拷贝一次cos sin（每个头cos sin都一样）
            ...
            for (uint32_t perSlice = 0; perSlice < sliceTimeQ_; ++perSlice) { // 核内每块
                ...
                CastF322F16(this->outQGm_[this->blockIdx_ * this->nlCoreRun_ * this->tilingData_->hiddenSizeQ +
                    zz * this->tilingData_->hiddenSizeQ + perSlice * sliceSizeTmp_],
                    commonUbuf_[resOut_],
            ...
            for (uint32_t perSlice = 0; perSlice < sliceTimeK_; ++perSlice) { // 核内每块
                ...
                CastF322F16(this->outKGm_[this->blockIdx_ * this->nlCoreRun_ * this->tilingData_->hiddenSizeK +
                    zz * this->tilingData_->hiddenSizeK + perSlice * sliceSizeTmp_],
