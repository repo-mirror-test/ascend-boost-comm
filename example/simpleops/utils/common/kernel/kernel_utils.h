...
__aicore__ inline void CopyIn(const AscendC::GlobalTensor<T> &gm, Q &queue, uint32_t offset, uint32_t count)
{
...
template <typename T, typename Q>
__aicore__ inline void CopyOut(const AscendC::GlobalTensor<T> &gm, Q &queue, uint32_t offset, uint32_t count)
{
...
__aicore__ inline void CopyInAndCastF32(const AscendC::LocalTensor<float> &out, const AscendC::GlobalTensor<T> &gm,
    Q &queue, uint32_t offset, uint32_t count)
{
...
template <typename T>
__aicore__ inline void ComputeRmsNorm(const AscendC::LocalTensor<T> &out, const AscendC::LocalTensor<float> &work,
...
{
    float value = 1.0;
    Duplicate(work, rms, count);
    AscendC::PipeBarrier<PIPE_V>();
    Div(work, tmp, work, count);
    AscendC::PipeBarrier<PIPE_V>();
    ...
    if (precisionMode == 0) {
        ...
        AscendC::PipeBarrier<PIPE_V>();
        if (gemmaMode == 1) {
            Adds(tmp, tmp, value, count);
        AscendC::PipeBarrier<PIPE_V>();
        }
        Mul(tmp, tmp, work, count);
        AscendC::PipeBarrier<PIPE_V>();
        CastFrom32To16(out, tmp, count);
        return;
    }
    if constexpr (std::is_same<T, half>::value) {
        CastFrom32To16(out, work, count);
        AscendC::PipeBarrier<PIPE_V>();
