...
void JsonToOpParam(const nlohmann::json &opDescJson, Mki::LaunchParam &launchParam)
{
    ...
    std::string opType = opDescJson["opType"];
    auto paramFunc = OP_DESC_JSON_FUNC_MAP.find(opType);
    if (paramFunc == OP_DESC_JSON_FUNC_MAP.end()) {
        MKI_LOG(ERROR) << "no opType: " << opType;
        return;
    }
    try {
...
def add_cast_types_code(code_lines, mem_type, mem_name):
    code_lines.append(f"    try {{")
    code_lines.append(f"        param.{mem_name} = static_cast<Mki::{mem_type}>(")
...
def add_cast_param_code(code_lines, struct_name, mem_type, mem_name):
    code_lines.append(f"    try {{")
    code_lines.append(f"        param.{mem_name} = static_cast<Mki::OpParam::{struct_name}::{mem_type}>(")
    code_lines.append(f"            opDescJson[\"specificParam\"].at(\"{mem_name}\"));")
    code_lines.append(f"    }} catch (...) {{")
    code_lines.append(f"        MKI_LOG(INFO) << \"{mem_name} not set, use default\";")
    code_lines.append(f"    }}")
...
def get_operation_json_func(operation_param_file):
    ...
    code_lines.append("{")
    code_lines.append(f"    Mki::OpParam::{struct_name} param;")
    for mem in mem_list:
        mem_type = mem[0]
        mem_name = mem[1]
        if mem_type == "bool":
...
def main():
    ...
    op_params_dir = os.path.join(current_dir, f"{os.environ['CODE_ROOT']}/include/mki/params")
