...
export MKI_DIR=$THIRD_PARTY_DIR/mki
...
BUILD_OPTION_LIST="testframework unittest pythontest alltest all_test_run debug release help dev clean"
BUILD_CONFIGURE_LIST=("--output=.*" "--cache=.*" "--mkidir=.*" "--incremental" "--gcov" "--force_clean" "--use_cxx11_abi=0"
...
function fn_build()
{
    ...
    COMPILE_OPTIONS="$COMPILE_OPTIONS -DOPS_OUTPUT_DIR=$OUTPUT_DIR"
    COMPILE_OPTIONS="$COMPILE_OPTIONS -DMKI_PACKAGE_DIR=$MKI_DIR"
...
function fn_main()
{
    ...
    do {
        ...
        --mkidir=*)
            arg2=${arg2#*=}
            if [ -z $arg2 ];then
                echo "the mkidir directory is not set. This should be set like --mkidir=<mkiDir>"
            else
                cd $CURRENT_DIR
                if [ ! -d "$arg2" ];then
                    echo "the mkidir directory is not set, building process will exit"
                    exit 1
                fi
                export MKI_DIR=$(cd $arg2; pwd)
            fi
            ;;
        --cache=*)
            arg2=${arg2#*=}
